...
//			line = resolveLine(line);
			...
			int nextSpace = line.indexOf(" ", firstPart.length());
			
			// filter to the next space, unless - the new value also contains a space, in which case do the EOL
			if (nextSpace == -1 || value.contains(" ")) {
				nextSpace = line.length();
			} else {
				nextSpace +=1;
			}
			
	//		if (firstPart.length() == nextSpace) {
	//			return line;
	//		}
			
			String existingValue = line.substring(firstPart.length()+1, nextSpace);
			if (existingValue.trim().length() == 0 && line.indexOf(" ", nextSpace+2) > -1) {
				existingValue = line.substring(firstPart.length(), line.indexOf(" ", nextSpace+2));				
			}
	...
	public String resolveLine(String line) {
		for (Map.Entry<String, String> entry : vars.entrySet()) {
			if (line.contains("$" + entry.getKey())) line = line.replace("$" + entry.getKey(),  entry.getValue());
		}
		return line;
	}
...
//					System.out.println("Ignoring substition:" + keyValueFile[0]);
	...
	String filterLine(String filename, String line) {
		for (Map.Entry<String, String> entry : vars.entrySet()) {
			if (this.varForFilename.containsKey(entry.getKey())){
				if (this.varForFilename.get(entry.getKey()).equals(filename)) {
					line = filterVariable(entry.getKey(), entry.getValue(), line);
				}
			} else {
