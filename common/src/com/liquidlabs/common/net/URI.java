...
public final class URI
    implements Comparable<URI>, Serializable, KryoSerializable
{
...

    ...
    private static final long L_USCORE = lowMask("_");
    private static final long H_DASH = highMask("-");
    private static final long H_USCORE = highMask("_");
...

	    ...
	    throws URISyntaxException
	{
		...
		if (q > p) {
		    p = q;
		    q = scan(p, n, L_ALPHANUM | L_DASH | L_USCORE, H_ALPHANUM | H_DASH | H_USCORE);
		    if (q > p) {
			if (charAt(q - 1) == '-')
    ...
    public void read(Kryo kryo, Input in) {
    	this.string = kryo.readObject(in, String.class);
    	try {
