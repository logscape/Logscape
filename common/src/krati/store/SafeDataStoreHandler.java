...
/**
 * SafeDataStoreHandler
 *
 * @author jwu
 *
 */
public final class SafeDataStoreHandler implements DataStoreHandler {
    private final static Logger _log = Logger.getLogger(SafeDataStoreHandler.class);
    ...
    public final List<byte[]> extractKeys(byte[] data) {
        try {
            ...
            int cnt = bb.getInt();
            if (cnt > data.length) {
                _log.error("Failed to extractKeys(): massive Count:" + cnt + " data:" + data.length);
                return null;
            }
    ...
    public final List<Entry<byte[], byte[]>> extractEntries(byte[] data) {
        try {
            ...
            int cnt = bb.getInt();
            if (cnt > data.length) {
                _log.error("Failed to extractEntries(), askedForSize:" + cnt + " available Length:" + data.length);
                return null;
            }
