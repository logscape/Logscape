...
public class Category implements AppenderAttachable {
	...
	static ThreadPoolExecutor runner = new ThreadPoolExecutor(1, 1,
  ...
  */
  ...
  public  void callAppenders(final LoggingEvent event) {
      // we could be running out of disk of something so stop logging msgs
      if (runner.getQueue().size() > 100) return;
	  ...
	  runner.execute(new Runnable() {
		...
		public void run() {
			  ...
			  for(Category c = Category.this; c != null; c=c.parent) {
			  ...
			  if(writes == 0) {
				  repository.emitNoAppenderWarning(Category.this);
			  }
		}
	  });
  }
