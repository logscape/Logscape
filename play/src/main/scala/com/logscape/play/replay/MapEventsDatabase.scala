 ...
 */
class MapEventsDatabase(parentDir:java.io.File, scheduler:ScheduledExecutorService, logSpace: LogSpace) extends Logging with EventsDatabase {

  val dbMap = new mutable.HashMap[String, PerRequestSessionDb]()
  ...
  def getDb(request:String) : PerRequestSessionDb = {
  ...
  private def getFieldSetForEvent(fieldSetId:String, db:PerRequestSessionDb) : FieldSet = {
  ...
  def getFieldSet(request:String) : FieldSet = {
    ...
    val sessionDb: PerRequestSessionDb = getDb(requestId)
  ...
  def getTypesForPage(events: Map[lang.Long, ReplayEvent], iterator: util.Iterator[lang.Long], start: Int, end: Int) = {
    ...
    while(iterator.hasNext ) {
      ...
      if (i >= start && i < end){
        val event: ReplayEvent = events.get(next).get
        val eventType : String = event.getDefaultField(FieldSet.DEF_FIELDS._type);
        set.+=(eventType)
