...
Private Sub ShowEvents(ByVal strMachine, _
 ...
 ' To set  GetSupportedUserLocale for Some  Diff locales 
    ...
    Do While (intLoopCount < objLogs.Count)
        ...
        'NEIL - fix to support large event log volumes required CInt to CLng changes below
            ...
            Else
                ...
                For each objInstance in objEnumerator
                    ...
                    If ( intResultCount >= intRecordRangeFrom ) And _
       			    ...
       			    ' HACK to Make sure the EVT Date format is correct (i.e. uses 24hour time format)
                            'is LOCALE CHANGED   
                            '     If bLocaleChanged <> TRUE Then
                            '          'format DatTime as DATE & "Space" & TIME    
                            '           arrResults(2)= "N" & Formatdatetime( Mid(strTemp,5,2) & "/"  & Mid(strTemp,7,2) & "/"  &_
