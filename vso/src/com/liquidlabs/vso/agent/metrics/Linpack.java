...
public class Linpack  implements ActionListener {
	...
	public static void main(String[] args) {
        ...
        int total = 0;
        // exclude the first
        new Linpack().actionPerformed(10 * 100);
        for (int i = 0; i < iterations; i++) {
            int actionPerformed = new Linpack().actionPerformed(10 * 100);
            System.out.println("MFLOPS:" + actionPerformed);
            total += actionPerformed;
        }
        System.out.println("AVG:" + (total / iterations));

        System.out.println("CORE_SCALED_MFLOPS:" + (total / iterations) * ManagementFactory.getOperatingSystemMXBean().getAvailableProcessors());
    }

	public void actionPerformed(ActionEvent e) {
	}

	public int actionPerformed(int psize) {
		// Object source = evt.getSource();

//		System.out.println("...running benchmark (" + psize + "x" + psize + ")");
		run_benchmark(psize, psize * 2);
		try {

			String s = ("http://www.netlib.org/cgi-bin/linpackjava/linpackjava.pl?"
			// String s =
			// ("http://netlib3.cs.utk.edu/cgi-bin/linpackjava/linpackjava.pl?"
					// + "email="
					// + URLEncoder.encode(email_e.getText())
					+ "&mflops=" + mflops_result + "&time=" + time_result
					// + "&cpu="
					...
					+ "&currtime=" + System.currentTimeMillis() + "&residn=" + residn_result + "&epsilon=" + eps_result + "&version=" + applet_version);

		//	System.out.println(s);

			return Double.valueOf(mflops_result).intValue();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return -1;
	...
	/**
	 * Same as normal action performed but takes an scaled average of the 2 values
	 */
	public int actionPerformed(int spsize, int oldValue, int updateCount) {
		int newMflops = this.actionPerformed(psize);
		if (oldValue == 0) return newMflops;
		return (newMflops + (oldValue * updateCount))/(updateCount+1);
	}
}
